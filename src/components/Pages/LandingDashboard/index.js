  ...
  useEffect(()=>{
    ...
    for(let i=0;i<listingData?.[TURING_CONSTANTS.NODES]?.length; i++){
      let item = {
        ...
        [TURING_CONSTANTS.DIRECTION]: <p style={{color:'blue'}}>{listingData?.[TURING_CONSTANTS.NODES][i]?.[TURING_CONSTANTS.DIRECTION]}</p>,
        ...
        <>
          ...
          <p style={{color: 'blue'}}>{'(' + listingData?.[TURING_CONSTANTS.NODES][i]?.[TURING_CONSTANTS.DURATION] + ') seconds'}</p>
      ...
      render: (address, record) => (
        ...
        <span 
          ...
          style={record?.[TURING_CONSTANTS.STATUS] ?
            {color:'teal', backgroundColor:'#F6F2F2'} : {color:'black', backgroundColor:'#C3C3C3'}}
